...
const RecipeEdite = () => {
  ...
  const recipeHandler = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data) {
        toast.success(res.data.message || "Recipe Edited successfully!");
        navigate("/");
      }
    } catch (error: any) {
      console.error(error);
  ...
  return (
    <section className="flex justify-center p-6">
      <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 text-white p-6 rounded-2xl">
        <h2 className="text-2xl mb-6 text-center">Edit Recipe</h2>
        ...
        <form className="space-y-5" onSubmit={recipeHandler}>
          ...
          <div className="flex flex-col">
            <label className="mb-1 text-sm">Instructions (period-separated)</label>
            <textarea
              required
              name="instructions"
          ...
          <div className="flex flex-col">
            <label className="mb-1 text-sm">Recipe Image (Leave blank to keep existing)</label>
            <input
              accept="image/*"
              type="file"
              name="file"
          ...
          >
            {isSubmitting ? "Saving Changes..." : "Edit Recipe"}
          </button>
        </form>
      </div>
    </section>
